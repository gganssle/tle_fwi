...
for i = 1:nz, j = 1:nx, k = 1:ny
	if ((i > 30) & (i <= floor(100 - 1.6 * j + 0.016 * j^2)) & (i <= floor(100 - 1.6 * k + 0.016 * k^2)))
		vel[i,j,k] = 2500
	end
end

for i = 1:nz, j = 1:nx, k = 1:ny
	if ((i > floor(100 - 1.6 * j + 0.016 * j^2)) & (i > floor(100 - 1.6 * k + 0.016 * k^2)) & (i <= floor(140 - 1.6 * j + 0.016 * j^2)) & (i <= floor(140 - 1.6 * k + 0.016 * k^2)))
		vel[i,j,k] = 3000
	end
end

for i = 1:nz, j = 1:nx, k = 1:ny
	if ((i < 170) & (i > floor(140 - 1.6 * j + 0.016 * j^2)) & (i > floor(140 - 1.6 * k + 0.016 * k^2)))
		vel[i,j,k] = 4000
	end
end
